...
// Typewriter Effect
const typewriterTexts = ['שבונה מותג', 'שמשדרג', 'שמחזק', 'שמביא לקוחות', 'שמייצר מכירות'];
...
function typewriterEffect() {
    ...
    if (!isDeleting && currentCharIndex < currentText.length) {
        ...
        dynamicTextElement.textContent = currentText.substring(0, currentCharIndex - 1);
        currentCharIndex--;
        typewriterTimeout = setTimeout(typewriterEffect, 75);
    } else {
        isDeleting = !isDeleting;
        if (!isDeleting) {
            currentTextIndex = (currentTextIndex + 1) % typewriterTexts.length;
        }
        typewriterTimeout = setTimeout(typewriterEffect, isDeleting ? 1500 : 800);
    }
}

// Enhanced Counter Animation - Removed
...
function initEnhancedScrollAnimations() {
    ...
    const revealElements = document.querySelectorAll('.service-card-premium, .package-ultra, .portfolio-item-premium');
...
function initEnhancedMobile() {
    if (window.innerWidth <= 768) {
        ...
        const interactiveElements = document.querySelectorAll('.service-card-premium, .package-ultra, .portfolio-item-premium');
